...
x_hs = tf.nn.tanh(tf.matmul(x_t, W_hs) + b_hs)
x_ht = tf.nn.tanh(tf.matmul(x_t, W_ht) + b_ht)
x_hr = tf.nn.tanh(tf.matmul(x_t, W_hr) + b_hr)

y_s = tf.nn.softmax(tf.matmul(x_hs, W_s) + b_s)
y_t = tf.nn.softmax(tf.matmul(x_ht, W_t) + b_t)
y_rp = tf.nn.softmax(tf.matmul(x_hr, W_rp) + b_rp)
...
for i in range(len(Class)):
  ...
  for j in range(len(W) / 3):
    ...
    if nd < dist:
      ...
      closest = j
      loc = l
  if D.brands[closest] in D.TrainingInput[i]['text']:
    found += 1
...
print found, len(Class), 100.0*found/len(Class)
print dirs
...
train_step_sfs = tf.train.GradientDescentOptimizer(lr).minimize(loss_sfs)
train_step_sft = tf.train.GradientDescentOptimizer(lr).minimize(loss_sft)
train_step_sfr = tf.train.GradientDescentOptimizer(lr).minimize(loss_sfr)
