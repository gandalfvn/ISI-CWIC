...
angular.module('app.generate').controller('genWorldCtrl', ['$rootScope', '$scope', '$state', '$stateParams', '$translate', '$window', '$localStorage', '$timeout', 'ngDialog', 'toaster', 'APP_CONST', 'ngTableParams', 'AppUtils', '$reactive', function($rootScope, $scope, $state, $stateParams, $translate, $window, $localStorage, $timeout, ngDialog, toaster, APP_CONST, ngTableParams, apputils, $reactive) {
  ...
  $scope.genStateN = function (params:miGen3DEngine.iMoveItr) {
    ...
    if ($scope.curState._id) {
      ...
      _.each(block_state, function (p, i) {
        ...
        var val:miGen3DEngine.iCubeState = {
          prop: {cid: p.id, size: size},
          position: <any>p.position,
          rotation: <any>p.rotation
        };
        used.push(val);
      ...
      var delta:{x:number, y:number, z:number} = {
      ...
      if (!$scope.curState._id && $scope.curState.block_states && $scope.curState.block_states.length
        ...
        var saveScreen = function (idx:number, list:iBlockStates, cb:(err?:any, savesid?:string)=>void) {
          ...
          ScreenCaps.insert({
            ...
            if(err) return console.warn('screencap Err', err);
        ...
        saveScreen(0, $scope.curState.block_states, function (err:any) {
          ...
          GenStates.insert(angular.copy($scope.curState), function (err:Error, id:string) {
            if(err) return console.warn('genstates Err', err);
  ...
  $scope.delMove = function (idx:number) {
    ...
    if($scope.curState._id){
      ...
      var id:string = $scope.curState._id;
      delete doc._id;
      GenStates.update({_id: id},{$set: doc}, function (err:Error, num:number) {
        if(err) return console.warn('delmove err:',err);
        $scope.clearMeta();
        $scope.showState(id);
      });
    }
  ...
  $scope.hideCube = function(cnt:number){
    if(cnt){
      var block_states = $scope.curState.block_states[$scope.createStateIdx];
  ...
  var radian = function(degrees:number):number{
    return degrees * Math.PI / 180;
  };

  $scope.rotCube = function(rotid:number, axis:string, deg:number){
    if(rotid){
      var rad = radian(deg);
      var c:miGen3DEngine.iMeshMod = myengine.get3DCubeById(rotid);
      if(c.isVisible)
